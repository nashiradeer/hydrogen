...
impl Lavalink {
    ...
    pub async fn update_player(
        ...
        let response = self
            .http_client
            .patch(self.config.build_rest_uri(&format!(
    ...
    pub async fn track_load(&self, identifier: &str) -> Result<TrackLoading> {
        let response = self
            ...
            .get(
                self.config
                    .build_rest_uri(&format!("/loadtracks?identifier={}", identifier,)),
            )
            .send()
            .await
            .map_err(Error::Reqwest)?
    ...
    pub async fn get_player(&self, guild_id: u64) -> Result<Player> {
        let response = self
            .http_client
            .get(self.config.build_rest_uri(&format!(
                "/sessions/{}/players/{}",
                self.session_id.read().unwrap().clone(),
                guild_id
            )))
    ...
    pub async fn destroy_player(&self, guild_id: u64) -> Result<()> {
        self.http_client
            .delete(self.config.build_rest_uri(&format!(
                "/sessions/{}/players/{}",
                self.session_id.read().unwrap().clone(),
                guild_id
            )))
