...
use serenity::{prelude::Context, model::prelude::{application_command::ApplicationCommandInteraction, GuildId, ChannelId, Guild, UserId}, builder::CreateApplicationCommand};
use songbird::{Songbird, Call};
use tokio::sync::Mutex;
use tracing::warn;

use crate::{HydrogenCommandListener, HydrogenContext, HYDROGEN_ERROR_COLOR, i18n::HydrogenI18n, HYDROGEN_LOGO_URL, HYDROGEN_PRIMARY_COLOR};
...
impl JoinCommand {
            ...
            Err(e) => {
                if let Err(e) = interaction.edit_original_interaction_response(&context.http, |response| {
                    response
                        .embed(|embed|
                            embed
                                .title(hydrogen.i18n.translate(&interaction.locale, "join", "embed_title"))
                                .description(hydrogen.i18n.translate(&interaction.locale, "join", "cant_connect"))
    ...
    async fn _execute(&self, hydrogen: HydrogenContext, context: Context, interaction: ApplicationCommandInteraction) -> Result<(), String> {
        ...
        let voice_manager = songbird::get(&context).await.ok_or("songbird not initialized".to_owned())?;
                    ...
                    .interaction_response_data(|data|
                        data
                            .embed(|embed|
                                embed
                                    .title(hydrogen.i18n.translate(&interaction.locale, "join", "embed_title"))
                                    .description(hydrogen.i18n.translate(&interaction.locale, "join", "player_exists"))
                                    .color(HYDROGEN_ERROR_COLOR)
                                    .footer(|footer|
                                        footer
                                            .text(hydrogen.i18n.translate(&interaction.locale, "embed", "footer_text"))
                ...
                if let Err(e) = interaction.edit_original_interaction_response(&context.http, |response| {
                    response
                        .embed(|embed|
                            embed
                                .title(hydrogen.i18n.translate(&interaction.locale, "join", "embed_title"))
                                .description(hydrogen.i18n.translate(&interaction.locale, "join", "unknown_voice_state"))
                                .color(HYDROGEN_ERROR_COLOR)
                                .footer(|footer|
                                    footer
                                        .text(hydrogen.i18n.translate(&interaction.locale, "embed", "footer_text"))
        ...
        if let Err(e) = interaction.edit_original_interaction_response(&context.http, |response| {
            response
                .embed(|embed|
                    embed
                        .title(hydrogen.i18n.translate(&interaction.locale, "join", "embed_title"))
                        .description(hydrogen.i18n.translate(&interaction.locale, "join", "success"))
