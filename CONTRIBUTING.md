...
First create your own branch following the [Hydrogen's Branch Naming Convention](#branches) and commits using the [Hydrogen's Commit Standard](#commits), remember that your branch needs to have only one change that change can affect more than one file and have more than one commit, but if your branch is related to how the commands is handled, you shouldn't fix a bug on the music player on the same branch.
